...
uint256 hashGenesisBlock("0x0000053d7472ea0e72b097daab87accb016ead050928e57d360a70d140ef78a6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CpiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CpiCoin Signed Message:\n";
...
{
    // CpiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // CpiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "CPI Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x66d43b9b99e51737f33145ada68f349813c6e789655030239662e208488f51ca"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc7, 0xb0, 0xde }; // CpiCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CpiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CpiCoinMiner : ProcessBlock, block not accepted");
    }
